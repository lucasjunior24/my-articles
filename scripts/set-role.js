 ...
 *
 * Uso:
 *   node scripts/set-role.js <uid-do-usuario> --role admin|writer
 *
 * Exemplo:
 *   node scripts/set-role.js abc123def456 --role admin
 *   node scripts/set-role.js abc123def456 --role writer
 *
 * Pré-requisitos:
...
if (!uid) {
  ...
  console.error("\nUso: node scripts/set-role.js <uid-do-usuario> --role admin|writer");
  console.error("Exemplo: node scripts/set-role.js abc123def456 --role admin\n");
  process.exit(1);
}
...
async function setRoleClaim() {
  ...
  try {
    ...
    // Se estiver setando admin, mantém writer se já existir
    // Se estiver setando writer, mantém admin se já existir
    const existingClaims = userRecord.customClaims || {};
    if (existingClaims.admin) claims.admin = true;
    if (existingClaims.writer) claims.writer = true;

    // Sobrescreve com a role desejada (garante que a nova claim seja setada)
    claims[role] = true;
